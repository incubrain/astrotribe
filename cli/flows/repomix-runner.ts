...
export class RepomixRunner {
  ...
  private async runConfig(config: RepomixConfig): Promise<boolean> {
    try {
  ...
  async getAvailableModes(): Promise<ModeInfo[]> {
    const configsDir = path.join(process.cwd(), 'repomix-configs')
  ...
  private async getConfigs(mode: string): Promise<RepomixConfig[]> {
    const configDir = path.join(process.cwd(), 'repomix-configs', mode)
