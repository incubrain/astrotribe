...
export async function isMcpServerConfigured(config: DebuggerConfig): Promise<boolean> {
  try {
    ...
    try {
      ...
      const vscodePathCmd =
        process.platform === 'win32' ? `where ${vscodeCommand} 2>&1` : `which ${vscodeCommand} 2>&1`
...
export async function ensureMcpServerConfigured(config: DebuggerConfig): Promise<boolean> {
  ...
  if (!isConfigured) {
    ...
    console.error(
      '2. Try using regular VS Code by setting USE_VSCODE_INSIDERS=false in your environment',
    )
    console.error(
      '3. Try using a direct MCP server URL with the MCP_SERVER_URL environment variable',
    )

    if (!config.disableSimulation) {
      console.warn(
        '⚠️ Running with simulated responses because MCP server is not properly configured',
      )
      return true
    }
