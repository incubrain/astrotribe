...
export function useHttpHandler() {
      ...
      try {
        ...
        if (
          ...
          const appError = new AppError({
            ...
            message:
              pgError.message ||
              retryableStatusCodes[Number(pgError.code) as keyof typeof retryableStatusCodes] ||
  ...
  async function select<T>(tableName: string, options: SelectOptions<T> = {}): Promise<T[]> {
    let query = supabase.from(tableName).select((options.columns as string) || '*')
    ...
    if (options.order) {
      query = query.order(options.order.column as string, { ascending: options.order.ascending })
    }

    return handleDatabaseOperation(async () => await query, `Select from ${tableName}`)
  }
