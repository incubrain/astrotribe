...
// Add this new function to your utils.ts
export async function checkAndSeedByContentType(
  pool: Pool,
  contentType: string,
  count: number,
  seederFn: (count: number) => Promise<any[]>,
): Promise<any[]> {
  try {
    // Check if table has data with the specific content_type
    const {
      rows: [{ count: existingCount }],
    } = await pool.query('SELECT COUNT(*) FROM contents WHERE content_type = $1', [contentType])

    if (Number(existingCount) > 0) {
      console.log(chalk.blue(`Using existing ${contentType} data in contents table`))
      const { rows } = await pool.query('SELECT * FROM contents WHERE content_type = $1', [
        contentType,
      ])
      return rows
    }

    // If no data exists for this content_type, run the seeder for just this type
    console.log(chalk.blue(`Seeding contents table with ${contentType} items...`))
    const result = await seederFn(count)
    console.log(chalk.green(`✓ Successfully seeded ${contentType} content`))
    return result
  } catch (error: any) {
    console.error(chalk.red(`Error while seeding ${contentType} content:`), error)
    throw error
  }
}
