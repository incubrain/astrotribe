...
export async function seedFollows(
  ...
  if (userIds.length > 1) {
    ...
    while (
      follows.length -
        (companyIds.length > 0 ? Math.min(count / 2, userIds.length * companyIds.length) : 0) <
        userFollowsUserCount &&
  ...
  try {
    ...
    console.log(
      `Inserting ${uniqueFollows.length} unique follows (filtered out ${follows.length - uniqueFollows.length} duplicates)`,
    )
