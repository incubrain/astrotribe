drop function if exists "public"."calculate_table_growth"(p_table_name text, p_time_period interval, p_num_periods integer);
...
SELECT cron.schedule('daily_database_stats', '0 0 * * 0', 'SELECT public.gather_database_stats()');
SELECT cron.schedule('monthly_cleanup', '0 3 1 * *', 'SELECT public.cleanup_table_stats()');
SELECT cron.schedule('weekly_maintenance', '0 1 * * 0', 'SELECT public.perform_weekly_maintenance()');
